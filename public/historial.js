document.addEventListener('DOMContentLoaded', () => {
  ...
  filtroFecha.addEventListener('change', async () => {
    ...
    const res = await fetch(`http://localhost:3000/historial/fecha/${fecha}`);
  ...
  selectCaso.addEventListener('change', async () => {
    ...
    const resCaso = await fetch(`http://localhost:3000/salidas/${id}`);
    ...
    // 🔴 HEMORRAGIA (si ocurrió alguna vez)
    ...
    const resHist = await fetch(`http://localhost:3000/historial/signos/${id}`);
