  ...
  recognition.onresult = (event) => {
    ...
    for (let key in keywords) {
      if (transcript.split(" ").includes(key)) {
        if (targetField) {
          const anterior = document.getElementById(targetField);
          if (anterior) anterior.classList.remove('active-field');
        }
    ...
    if (targetField) {
      ...
      let cleanRegex = new RegExp(
        "(nombre|edad|ubicación|ubicacion|informe|diagnóstico|diagnostico|sangre|sexo|carnet|traslado)",
        "gi"
      );
...
function irMonitoreo() {
  if (!localStorage.getItem('salida_activa')) {
    return alert('⚠️ Registra al paciente primero');
  }
